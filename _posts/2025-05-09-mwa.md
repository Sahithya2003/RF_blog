...
Funny thing I noticed — when I moved my slots closer to the tip and **kept only the first set inside the tumor**, my S11 improved dramatically (like from –7 dB to –12 dB). And weirdly — when the second set of slots was **outside** the tumor (as seen in the model), the performance still improved. Why? 
Turns out, the slots closest to the **tissue boundary (tumor–liver)** interact with both dielectrics, and this “contrast zone” can sometimes help in impedance matching — especially if the antenna is radiating more efficiently near the high-εr tumor but leaking gently into the liver. It’s like the antenna’s field is softly tapering instead of sharply ending. 
...
Naturally, I did what any curious person would do — I started Googling. That led me to a bunch of IEEE papers on microwave ablation. One in particular described a dual-slot coaxial antenna for liver tumor ablation. That paper didn’t just answer my question — it turned into the basis of this whole project. 
...
Originally, I gave the cone tip only 1 mm height — way too small to radiate efficiently in tissue. The fields looked weak, and SAR dropped off fast. Once I adjusted the cone to 3 mm (about λ/4 in tissue), the SAR distribution improved significantly. 
...
``` 
The results? The CEM43 passed 240 within 6 mm of the tip after 5 minutes. We’re not just heating tissue — we’re cooking it to clinical standards. 
